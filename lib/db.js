...
function getConnection(type) {
    ...
    const readonly = (type !== 'data' && type !== 'cache');
...
function searchMovies(searchQuery, limit = 50) {
  ...
  try {
    ...
    const createTermConditions = (terms) => terms.map(() => `lower_utf8(title) LIKE '%' || ? || '%'`).join(' AND ');
    ...
    const sqlQuery = `
      ...
      AND ((${createTermConditions(rawTerms)}) OR (${createTermConditions(fixedTerms)}))
      ORDER BY search_rank ASC, vote_count DESC, vote_average DESC
      LIMIT ?
    `;

    const params = [rawQueryLower, rawQueryLower, ...rawTerms, ...fixedTerms, limit];
...
async function getTopRatedMoviesApi() {
    return getMoviesFromApiCache('home_top_rated', '/movie/top_rated', 86400);
}
async function getRecommendationsApi(tmdbId) {
    const ids = await fetchMultiplePages(`/movie/${tmdbId}/recommendations`, 3, 604800);
...
async function fetchMultiplePages(endpoint, pages = 3, ttlSeconds = 86400) {
    let allIds = [];
